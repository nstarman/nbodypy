...
def sphere_coords(cluster):
    ...
    return cart_to_sphere(
        cluster.x, cluster.y, cluster.z, cluster.vx, cluster.vy, cluster.vz
    )
...
def cart_to_sphere(x, y, z, vx, vy, vz):
    ...
    thatz = -1.0 * np.sin(theta)

    phatx = -1.0 * np.sin(phi)
    phaty = np.cos(phi)
    phatz = 0.0
...
def sky_coords(cluster):
    ...
    pmra, pmdec = bovy_coords.pmllpmbb_to_pmrapmdec(
