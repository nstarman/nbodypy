...
def extrct_out(cluster, fileout, projected=False):
    ...
    if cluster.ntot == 0:
        ...
        trh = half_mass_relaxation_time(
            cluster, multimass=True, projected=projected
        )
    ...
    fileout.write(
        "%i %i %f %f %f "
        % (cluster.ntot, nb, cluster.tphys, trh, cluster.mtot)
    )
...
def fortout(
    ...
    if reset_nbody_scale:
        reset_nbody_scale(
            cluster, mass=reset_nbody_mass, radii=reset_nbody_radii
        )
